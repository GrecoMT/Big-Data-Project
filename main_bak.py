'''
BACKUP DELLA CLASSE MAIN IN CASO DOVESSERO ESSERCI PROBLEMI
...
datasetPath = "/Users/vincenzopresta/Desktop/Big Data/dataset/Hotel_Reviews.csv"
...
class QueryManager:
    ...
    def reviewScoreDisribution(spark):
        spark.df_finale.groupBy("Reviewer_Score").count().orderBy("Reviewer_Score").show()
...
'''if __name__ == '__main__':
    ...
    #Controllo se ci sono ancora righe vuote o valori null nelle colonne chiave
    spark.df_finale.select([F.count(F.when(F.col(c).isNull(), 1)).alias(c) for c in spark.df_finale.columns]).show()
    
    #Controllo della correttezza delle coordinate
    spark.df_finale.filter((F.col("lat") < -90) | (F.col("lat") > 90) | (F.col("lng") < -180) | (F.col("lng") > 180)).show()

    #Dai due sopra emerge che i dati sono completi

    #Visualizzo un campione dei dati finali:
    spark.df_finale.show(10, truncate=True)'''
